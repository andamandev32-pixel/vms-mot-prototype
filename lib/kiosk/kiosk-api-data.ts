...
const apiSpecs: KioskApiSpec[] = [
    ...
    endpoints: [
      {
        ...
        summary: "โหลด config ตู้ + เช็คสถานะ online + เวลาทำการ",
        summaryEn: "Load kiosk config + check online status + business hours",
    ...
    endpoints: [
      ...
      },
      {
        ...
        notes: [
          ...
          "ตรวจจาก visit_records ที่สร้างภายในวันนี้ + status = checked-in",
        ...
        notesEn: [
          ...
          "Check visit_records created today with status = checked-in",
  ...
  // ─── SUCCESS ───
  {
    ...
    description: "API หลัก — สร้าง visit_record + ออก QR/Slip + ส่ง notification + sync access control (Hikvision)",
    descriptionEn: "Main API — create visit_record + generate QR/Slip + send notifications + sync access control (Hikvision)",
    hasApi: true,
    endpoints: [
      {
        method: "POST",
        path: "/api/kiosk/checkin",
        summary: "สร้าง visit_record + ออก QR/Slip + ส่ง notification + sync Hikvision",
        summaryEn: "Create visit record + generate QR/Slip + send notifications + sync Hikvision",
        tables: [
          "visit_records", "visitors", "visit_purposes", "departments",
        ...
        notesEn: [
          "★ Main API — combines all transactions in one call",
          "Hikvision sync is async on backend (doesn't block kiosk)",
          "Use booking_code as idempotency key — prevent double checkin",
      ...
      },
      {
        ...
        request: { visitRecordId: 99, printed: true, printMethod: "thermal-80mm" },
    ...
    configSources: [
      {
        ...
        fields: ["visit_slip_templates.size", "visit_slip_templates.fields", "visit_slip_templates.header_text", "visit_slip_templates.footer_text"],
        usage: "กำหนดรูปแบบ slip: ขนาด (thermal-80mm), field ที่แสดง, header/footer",
        usageEn: "Configure slip template: size (thermal-80mm), displayed fields, header/footer",
      },
      {
      ...
      },
      {
        ...
        settingsPageEn: "Service Points",
        settingsPath: "/web/settings/service-points",
        fields: ["service_points.slip_config.header_text", "service_points.slip_config.footer_text"],
        usage: "Override slip header/footer ต่อ Kiosk",
        usageEn: "Per-kiosk slip header/footer override",
      },
    ],
    devNotes: [
    ...
    endpoints: [
      {
        ...
        tables: ["visit_records", "visitors", "staff", "departments", "visit_purposes", "floors"],
        request: {
        ...
        notesEn: [
          "Check approved status before check-in",
    ...
    endpoints: [
      {
        ...
        summary: "ยืนยันตัวตนกับนัดหมาย + ตรวจ Blocklist",
        summaryEn: "Verify identity against appointment + check Blocklist",
        ...
        notes: [
          "ตรวจ Blocklist + match กับข้อมูลนัดหมาย",
          "เลือกวิธียืนยัน + อ่านข้อมูลอัตโนมัติ (auto-read) ใน 1 ขั้นตอน",
          "ถ้าไม่ตรง → แสดง error + ให้ลองใหม่",
        ],
        notesEn: [
          "Check Blocklist + match against appointment data",
          "Select method + auto-read data in 1 step",
