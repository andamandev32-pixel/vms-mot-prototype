  ...
  endpoints: [
    {
      ...
      summaryEn: "Login (supports username or email)",
      auth: "public",
      requestBody: [
        { name: "usernameOrEmail", type: "string", required: true, description: "ชื่อผู้ใช้ หรือ อีเมล" },
  ...
  "user": {
    ...
    "refId": 5
      ...
      notes: [
        ...
        "ตรวจ isActive = true ก่อน (inactive/locked → 403)",
        "อัปเดต lastLoginAt",
        "คืน JWT token ทั้งใน response body และ httpOnly cookie (evms_session)",
        "token หมดอายุ 8 ชั่วโมง",
    ...
    },
    {
      ...
      summary: "สมัครสมาชิก (ลงทะเบียนแล้วใช้งานได้ทันที)",
      summaryEn: "Register (auto-login, no email verification)",
      auth: "public",
      requestBody: [
        ...
        { name: "phone", type: "string", required: true, description: "เบอร์โทรศัพท์ (unique)" },
        ...
        { name: "departmentId", type: "number", required: false, description: "รหัสแผนก (required ถ้า staff)" },
        { name: "position", type: "string", required: false, description: "ตำแหน่ง (required ถ้า staff)" },
        { name: "lineAccessToken", type: "string", required: false, description: "LIFF access token สำหรับผูก LINE account (optional, ใช้เมื่อลงทะเบียนจาก LIFF)" },
      ],
      responseExample: `{
  "token": "eyJhbGciOiJIUzI1NiIs...",
  "user": {
    ...
    "departmentName": null
      ...
      notes: [
        "ลงทะเบียนแล้วใช้งานได้ทันที (auto-login, ไม่ต้อง verify email)",
        "คืน JWT token ทั้งใน response body และ httpOnly cookie",
        "role = userType (visitor → visitor, staff → staff)",
        "visitor ต้องระบุ idNumber + idType",
        "staff ต้องระบุ employeeId + departmentId + position",
        "ตรวจ uniqueness: email, phone, username (ถ้ามี)",
      ],
    },
    {
      method: "POST",
    ...
    },
    {
      ...
      summaryEn: "Get current user session",
      auth: "user",
      responseExample: `{
  "user": {
    "id": 1,
    ...
    "refId": 5
      ...
      notes: [
        "รองรับทั้ง cookie (evms_session) และ Authorization: Bearer <token>",
        "คืน 401 ถ้าไม่มี token หรือ token หมดอายุ",
      ],
    },
    {
      method: "GET",
      ...
      summary: "ออกจากระบบ",
      summaryEn: "Logout (clear session cookie)",
      auth: "public",
      responseExample: `{ "success": true }`,
      notes: ["ลบ cookie evms_session (maxAge: 0)"],
    },
  ],
};
...
const servicePointsApi: PageApiDoc = {
  ...
  "data": [
    ...
    },
    {
      method: "POST",
      path: "/api/service-points",
      summary: "สร้างจุดบริการใหม่ (ถ้า type=kiosk จะ auto-generate Device Token)",
      summaryEn: "Create service point (auto-generates Device Token if type=kiosk)",
      auth: "admin",
      requestBody: [
        { name: "name", type: "string", required: true, description: "ชื่อจุดบริการ" },
        { name: "nameEn", type: "string", required: true, description: "ชื่อจุดบริการ (English)" },
        { name: "type", type: "string", required: true, description: "kiosk | counter" },
        { name: "serialNumber", type: "string", required: true, description: "หมายเลขซีเรียล (unique)" },
        { name: "location", type: "string", required: true, description: "ตำแหน่งที่ตั้ง" },
        { name: "locationEn", type: "string", required: true, description: "ตำแหน่งที่ตั้ง (English)" },
        ...
        { name: "macAddress", type: "string", required: true, description: "MAC address" },
      ],
      responseExample: `{
  "servicePoint": { "id": 1, "name": "Kiosk ล็อบบี้ ชั้น 1", "type": "kiosk" },
  "deviceToken": "kvms_a1b2c3d4e5f6...",
  "kioskDevice": { "id": 1 }
}`,
    },
    {
