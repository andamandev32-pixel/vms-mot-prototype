...
export const lineFlowStates: FlowStateInfo[] = [
  ...
  {
    id: "visitor-approval-rejected",
    name: "แจ้งผลไม่อนุมัติ",
    nameEn: "Approval Rejected Notification",
    description: "เจ้าหน้าที่กดไม่อนุมัติพร้อมระบุเหตุผล → ระบบส่ง Flex Message แจ้ง visitor ทาง LINE พร้อมเหตุผลและปุ่มสร้างนัดหมายใหม่",
    userType: "visitor",
    order: 6.2,
    triggers: ["Officer rejects via LINE or Web", "POST /api/appointments/:id/reject"],
    nextStates: ["visitor-booking"],
    relatedChannels: ["line", "web"],
    roles: ["visitor", "officer", "supervisor"],
    dbTables: ["appointments", "notification_logs"],
    apiEndpoints: ["POST /api/appointments/:id/reject", "POST /api/line/push-message"],
  },
  {
  ...
  {
    id: "visitor-overstay-alert",
    name: "แจ้งเตือน Visitor อยู่เกินเวลา",
    nameEn: "Visitor Overstay Reminder",
    description: "ถ้า visitor ยังไม่ check-out เกินเวลาที่กำหนด → cron job ส่ง Push Message เตือนทาง LINE ให้รีบ check-out ที่ counter",
    userType: "visitor",
    order: 10.5,
    triggers: ["Scheduled cron job ตรวจ visit_entries ที่ checked-in เกินเวลา", "checkOverstayEntries() in lib/overstay-checker.ts"],
    nextStates: ["visitor-checkout"],
    relatedChannels: ["line"],
    roles: ["visitor"],
  ...
  // ─── Officer Flow ───
  {
    ...
    description: "เปิด LIFF App → เลือก 'พนักงาน' → ค้นหาด้วยชื่อ-นามสกุล/รหัสพนักงาน/เลขบัตร → เลือกรายชื่อของตน (เมื่อชื่อซ้ำ) → กรอกข้อมูลสร้างบัญชี (สิทธิ์/username/password) + ผูก LINE → เปลี่ยน Rich Menu เป็น Officer Menu",
    userType: "officer",
    order: 12,
    ...
    codeExample: [
      ...
      "// Step 1: ค้นหาพนักงาน (ชื่อ-นามสกุล / รหัสพนักงาน / เลขบัตรประชาชน)",
      "const res1 = await fetch('/api/auth/check-staff', {",
      "  method: 'POST',",
      "  body: JSON.stringify({ query: 'สมศรี รักงาน' }),",
      "});",
      "const { candidates } = (await res1.json()).data;",
      "// ชื่อซ้ำ → ให้ผู้ใช้เลือกจากรายการก่อน",
      "const personnel = candidates[0];",
      "",
      "// Step 2: ลงทะเบียน + ตั้งสิทธิ์/username/password + ผูก LINE",
      "const res2 = await fetch('/api/auth/register', {",
      "  method: 'POST',",
      "  body: JSON.stringify({",
      "    userType: 'staff',",
      "    role: 'staff',            // staff | supervisor | security",
      "    username: 'officer01',    // สำหรับ login Web App",
      "    password: 'MyPass@123',   // ≥ 8 ตัว + A-Z a-z 0-9 อักขระพิเศษ",
...
export const lineApiEndpoints: LineApiEndpoint[] = [
  ...
  // ─── Staff Lookup (Officer Registration) ───
  {
    ...
    summary: "ค้นหาข้อมูลพนักงานด้วยชื่อ-นามสกุล รหัสพนักงาน หรือ เลขบัตรประชาชน",
    summaryEn: "Lookup Staff by Name, Employee ID or National ID",
    auth: "public",
    queryParams: [
      { name: "query", type: "string", required: true, description: "ชื่อ-นามสกุล / รหัสพนักงาน / เลขบัตรประชาชน" },
