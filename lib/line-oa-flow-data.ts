...
export const lineFlowStates: FlowStateInfo[] = [
  ...
  },
  {
    ...
    description: "เปิด LIFF → เลือกวัตถุประสงค์ (filter showOnLine=true) → เลือกวัน/เวลา → เลือกเจ้าหน้าที่ → กรอกรายละเอียด → ยืนยัน PDPA → ส่งคำขอ",
    userType: "visitor",
    order: 4,
    ...
    roles: ["visitor"],
    dbTables: ["appointments", "visit_purposes", "visit_purpose_department_rules", "pdpa_consents"],
    apiEndpoints: ["GET /api/visit-purposes?channel=line", "GET /api/staff", "POST /api/appointments", "POST /api/pdpa-consents"],
    codeExample: [
      "// LIFF Booking (Frontend)",
  ...
  },
  {
    ...
    description: "เมื่อ visitor สร้างนัดหมายที่ต้อง approval → ระบบส่ง Flex Message ไปยัง host officer + approver group → แสดงรายละเอียด visitor + วัตถุประสงค์ + ปุ่ม อนุมัติ/ปฏิเสธ",
    userType: "officer",
    order: 14,
    triggers: ["POST /api/appointments creates new appointment with require_approval=true"],
    nextStates: ["officer-approve-action"],
    relatedChannels: ["line", "web"],
    roles: ["officer", "supervisor"],
    dbTables: ["appointments", "approver_groups", "notification_logs"],
    apiEndpoints: ["POST /api/line/push-message", "GET /api/appointments/:id"],
  },
  {
    id: "officer-approve-action",
    name: "อนุมัติ / ปฏิเสธ",
    nameEn: "Approve or Reject Action",
    description: "Officer กดปุ่ม 'อนุมัติ' หรือ 'ปฏิเสธ' บน Flex Message → เปิด LIFF เพื่อยืนยัน → ระบบอัปเดตสถานะ → แจ้ง visitor",
    userType: "officer",
    order: 15,
    triggers: ["Officer taps approve/reject on Flex Message", "LINE Postback Event"],
  ...
  },
  {
    ...
    description: "เมื่อ visitor check-in ที่ kiosk/counter → ระบบส่ง Push Message แจ้ง host officer → แสดงชื่อ visitor, เวลา check-in, สถานที่",
    userType: "officer",
    order: 16,
    triggers: ["POST /api/kiosk/checkin or /api/counter/checkin success"],
