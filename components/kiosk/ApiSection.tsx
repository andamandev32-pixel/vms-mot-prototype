...
import { getApiSpec, type KioskApiEndpoint } from "@/lib/kiosk/kiosk-api-data";
import type { KioskStateType, KioskLocale } from "@/lib/kiosk/kiosk-types";
...
function EndpointCard({ endpoint, locale, isOnly }: { endpoint: KioskApiEndpoint; locale: KioskLocale; isOnly: boolean }) {
  ...
  const notes = locale === "th" ? endpoint.notes : endpoint.notesEn;
  ...
  return (
    <div className={cn("space-y-2", !isOnly && "pt-2 border-t border-gray-800 first:border-t-0 first:pt-0")}>
      ...
      <p className="text-[10px] text-gray-400">
        {locale === "th" ? endpoint.summary : endpoint.summaryEn}
      </p>
      ...
      {activeTab === "error" && endpoint.errorResponse && (
        <JsonBlock data={endpoint.errorResponse} />
      )}
...
export default function ApiSection({ stateType, locale }: ApiSectionProps) {
  const spec = getApiSpec(stateType);
